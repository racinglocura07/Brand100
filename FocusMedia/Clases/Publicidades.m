...
//  FocusMedia
//
//  Created by Administrador on 19/4/16.
//  Copyright © 2016 No Fue Magia. All rights reserved.
//

#import "Publicidades.h"

@interface Publicidades ()
...
- (void)viewDidLoad {
    ...
    NSString * rutaImagenes =[NSString stringWithFormat:@"%@/%@", [[NSBundle mainBundle] resourcePath], sharedManager.PublicidadesUrl ];
    
    if (sharedManager.TodoDescargado){
        rutaImagenes = [NSString stringWithFormat:@"%@%@", sharedManager.RutaDescarga,sharedManager.PublicidadesUrl];
    }
...
- (void)setImage:(NSInteger)tag{
    NSString * tmp =[_planosArray objectAtIndex:tag];
